...
class EncoderExtractor:
    ...
    def process_directory(self, input_dir, output_dir, max_images_per_class=None):
        """
...
def main():
    ...
    parser.add_argument('--max_images_per_class', type=int, default=None,
                       help='Maximum number of images to process per class (None for all)')
    ...
    if args.max_images_per_class:
        logger.info(f"Max images per class: {args.max_images_per_class}")
    
    # Initialize extractor
    extractor = EncoderExtractor(args.model_path, device)
    
    # Process directory
    extractor.process_directory(args.input_dir, args.output_dir, args.max_images_per_class)
