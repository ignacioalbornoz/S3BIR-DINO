...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor], prompt_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        ...
        for t_x, t_masks, t_prompt in zip(x_list, masks_list, prompt_list):
            t2_x, hw_tuple = self.prepare_tokens_with_masks(t_x, t_masks, t_prompt)
            x.append(t2_x)
...
class S3birDinov3(nn.Module):
    ...
    def forward(self, data, dtype='image', is_training=False):
        if dtype == 'image':
            feat = self.encoder(data, prompt=self.img_prompt.expand(data.shape[0], -1, -1), is_training=is_training)
        else:
            feat = self.encoder(data, prompt=self.sk_prompt.expand(data.shape[0], -1, -1), is_training=is_training)
        return feat