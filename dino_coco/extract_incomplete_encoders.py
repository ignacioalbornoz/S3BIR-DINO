#!/usr/bin/env python3
"""
Script to extract encoders from S3BIR-DINOv3 model for INCOMPLETE classes.
Processes classes that exist but have missing encoders, including the last incomplete class.
"""
...
class IncompleteEncoderExtractor:
    ...
    def get_incomplete_classes(self, input_dir, output_dir):
        """Get list of classes that are incomplete (missing some encoders)."""
        ...
        for class_dir in class_dirs:
            ...
            if len(output_encoders) < len(input_images):
                missing_count = len(input_images) - len(output_encoders)
                incomplete_classes.append((class_name, len(input_images), len(output_encoders), missing_count))
                logger.info(f"Incomplete class: {class_name} ({len(output_encoders)}/{len(input_images)} encoders, {missing_count} missing)")
        
        return missing_classes, incomplete_classes
    
    def process_incomplete_classes(self, input_dir, output_dir):
        """
        Process only the classes that are incomplete.
        ...
        missing_classes, incomplete_classes = self.get_incomplete_classes(input_dir, output_dir)
        ...
        for class_name, total_images, existing_encoders, missing_count in incomplete_classes:
            logger.info(f"Processing incomplete class: {class_name} ({existing_encoders}/{total_images} done, {missing_count} missing)")
            class_dir = input_path / class_name
            class_output_dir = output_path / class_name
...
def main():
    parser = argparse.ArgumentParser(description='Extract encoders for incomplete classes from S3BIR-DINOv3 model')
    ...
    extractor = IncompleteEncoderExtractor(args.model_path, device)
    
    # Process incomplete classes
    extractor.process_incomplete_classes(args.input_dir, args.output_dir)
